...
- **v1.5** — 状态广播闭环 + 自动救援 + 维护性改进 (Phases 15-17) ✓ COMPLETE
- **Next:** 待规划
...
**Delivered:**
- `scripts/claude_auto_rescue.sh` (300 lines) - Auto-confirmation script
- Commands: `check`, `run`, `status` with `--session` option
- Worker-only enforcement (worker-0/1/2, master excluded)
- 30-second cooldown per window
- Dangerous command blacklist (rm -rf, DROP, DELETE, sudo, etc.)

**Status:** ✓ Verified 2026-02-02

**Design Specs:**

1. **Pattern Detection Categories:**
   ```
   Priority 1 (Dangerous - BLOCK): rm -rf, rm -r, rm -f, del, delete, drop, DROP, truncate, sudo, password, passwd
   Priority 2 (Prompts - AUTO-CONFIRM): [y/n], [Y/n], [y/N], press enter, press return, hit enter, 回车继续, 按回车
...
3. **Worker-Only Restriction:**
   ```bash
   is_worker_window() {
       [[ "$window" == worker-0 ]] || [[ "$window" == worker-1 ]] || [[ "$window" == worker-2 ]]
   }
   ```
...
**Delivered:**
- `scripts/swarm_broadcast.sh` - Status broadcasting wrapper with `start/done/error/wait` commands
...
- CONTRIBUTING.md - Script conventions and testing requirements documentation

**Status:** ✓ Verified 2026-02-02

**Design Specs:**

1. **Worker Auto-Detection:**
   ```bash
   # Uses tmux display-message to get window name
   tmux display-message -p -t "$TMUX_PANE" '#{window_name}'
   # Validates worker-0/1/2 only
   ```

2. **Status Broadcasting Commands:**
   ```
   start <task_id> [reason]  # Record START status
   done <task_id> [reason]   # Record DONE status
   error <task_id> <reason>  # Record ERROR status
   wait <task_id> <reason>   # Record WAIT status
   ```

3. **Integration with swarm_status_log.sh:**
   - swarm_broadcast.sh is a simplified wrapper
   - All status records use JSON Lines format via swarm_status_log.sh append
   - Timestamp generated by swarm_status_log.sh (not by swarm_broadcast.sh)
