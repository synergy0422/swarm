...
2. **Configuration variables:**
   ...
   - `WORKDIR="${AI_SWARM_DIR:-/home/user/projects/AAA/swarm}"` (configurable via env var)
...
5. **Tmux session creation:**
   ...
   - Launch claude in each window using `tmux send-keys -t "$SESSION_NAME:window" "claude" Enter`
...
3. **Check Claude is running in each window:**
   ...
   ```
   Each should show `claude` as the current command
