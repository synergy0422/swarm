...
files_modified:
  ...
  - /home/user/projects/AAA/swarm/tests/test_master_tmux_scan.py
autonomous: true
must_haves:
  truths:
...
2. Add `pane_poll_interval: float = 3.0` parameter (internal use, NOT exposed to CLI)
...
**Note on pane_poll_interval:**
- **v1.1 不暴露 CLI 参数**，仅内部默认 3 秒
- 可通过 `Master(pane_poll_interval=X)` 构造参数调整
- 后续版本可考虑 `--pane-interval` CLI 参数
