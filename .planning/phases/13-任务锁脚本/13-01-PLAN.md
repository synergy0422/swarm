...
2. `./scripts/swarm_lock.sh acquire task-001 worker-0` (again) → fails with error message
...
5. `./scripts/swarm_lock.sh check <task_id>` shows active/expired status
6. `./scripts/swarm_lock.sh list` shows all locks with status
...
- **Handle expired locks:** Check if lock file exists and is expired, if so → os.unlink(lock_file) to delete it first
- **Atomic create:** Use python os.open(O_CREAT|O_EXCL) for race-condition-safe creation:
  ```python
  python3 - <<'PY'
  import os, json, sys
  ...
  if os.path.exists(lock_file):
      ...
      # Check if expired (expires_at is present and in the past)
  ...
  # Atomic create (fails if file still exists)
  try:
...
- Write JSON lock file with all 4 fields (task_id, worker, acquired_at, expires_at)
- Output human-readable success message
- Handle error cases: missing arguments, lock exists (non-expired)

**Acceptance:**
- `./scripts/swarm_lock.sh acquire task-001 worker-0` creates lock file
- `./scripts/swarm_lock.sh acquire task-001 worker-0` (again) fails with error
- Expired lock can be re-acquired (expired lock is deleted automatically)
