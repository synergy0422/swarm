...
1. `swarm_lock.sh acquire task-001 worker-0` creates lock file and returns lock content
2. Lock exists → repeated acquire fails with error message
3. `swarm_lock.sh release task-001 worker-0` successfully deletes lock when owner matches
...
1. **Acquire:**
   - Fail immediately if lock exists (atomic via mkdir -p with O_EXCL semantics)
...
**File:** `/home/user/projects/AAA/swarm/swarm_lock.sh`
...
- Use `mkdir -p "$LOCK_DIR"` to create lock directory
- Use atomic create pattern (fail if exists)
...
**Acceptance:**
- `swarm_lock.sh list` shows all locks with status
...
swarm_lock.sh acquire task-001 worker-0
...
swarm_lock.sh check task-001 | grep -q "Active" && echo "PASS: Shows active" || echo "FAIL: Not active"

# Test 4: Release with correct worker
echo "Test 4: Release with correct worker"
swarm_lock.sh release task-001 worker-0
...
swarm_lock.sh release task-001 wrong-worker
[ $? -ne 0 ] && echo "PASS: Release failed as expected" || echo "FAIL: Should have failed"

# Test 7: List shows locks
echo "Test 7: List locks"
swarm_lock.sh list | grep -q "task-001" && echo "PASS: List shows task" || echo "FAIL: List missing task"

# Test 8: TTL behavior
echo "Test 8: TTL with expiry"
rm -rf /tmp/ai_swarm/locks
swarm_lock.sh acquire task-002 worker-0 1  # 1 second TTL
sleep 2
swarm_lock.sh check task-002 | grep -q "Expired" && echo "PASS: Lock expired" || echo "FAIL: Lock not expired"

# Test 9: Expired lock can be re-acquired
echo "Test 9: Re-acquire expired lock"
swarm_lock.sh acquire task-002 worker-0
...
- Lock acquisition uses mkdir -p with O_CREAT|O_EXCL semantics for atomicity
