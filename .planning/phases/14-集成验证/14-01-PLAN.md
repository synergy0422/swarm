...
- **State directory:** Uses `SWARM_STATE_DIR` (default: `/tmp/ai_swarm`)
- **Isolation:** Each test run uses a unique test task_id to avoid conflicts
- **Cleanup:** Test artifacts are cleaned up after verification
...
3. Create cleanup function:
   ```bash
   cleanup() {
       rm -f "$LOCK_DIR/$TEST_TASK_ID.lock" 2>/dev/null || true
       rm -f "$STATE_DIR/status.log" 2>/dev/null || true
   }
   trap cleanup EXIT
   ```
...
swarm_e2e_test.sh ──────> /tmp/ai_swarm/ (Verifies shared state between scripts)
```
...
- Test uses unique task_id with timestamp to avoid conflicts
- Cleanup is automatic via trap on EXIT
