...
must_haves:
  truths:
    ...
    - "AI_SWARM_AUTO_RESCUE_BLOCK regex patterns block matching content"
    - "AI_SWARM_AUTO_RESCUE_ALLOW regex patterns whitelist matching content"
...
2. If `BLOCK` pattern matches → return `(True, 'dangerous_blocked', 'BLOCK pattern matched')`
3. If `ALLOW` is set and no match → return `(False, 'manual_confirm_needed', 'ALLOW pattern not matched')`
...
if self.block_pattern and pane_output:
    ...
    if block_match:
        ...
        return True, 'dangerous_blocked', f'BLOCK pattern matched: {block_match.group(0)}'
...
if self.allow_pattern and pane_output:
    ...
    if not allow_match:
        ...
        return False, 'manual_confirm_needed', 'ALLOW pattern not matched'
```
...
3. Test with BLOCK pattern:
   ...
   - Should return `(True, 'dangerous_blocked', 'BLOCK pattern matched: test content')`
...
4. Test with ALLOW pattern (no match):
   ...
   - Should return `(False, 'manual_confirm_needed', 'ALLOW pattern not matched')`
...
- [ ] `AI_SWARM_AUTO_RESCUE_BLOCK` regex blocks matching content
- [ ] `AI_SWARM_AUTO_RESCUE_ALLOW` regex whitelists content (manual confirm if no match)
