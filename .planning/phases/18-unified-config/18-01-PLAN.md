...
must_haves:
  truths:
    ...
    - "Graceful degradation if _config.sh missing (uses defaults)"
...
1. Source _config.sh at the top (after source guard):
   ```bash
...
cd /tmp && \
  test -f /home/user/projects/AAA/swarm/scripts/_config.sh && \
  grep "_config.sh" /home/user/projects/AAA/swarm/scripts/_common.sh >/dev/null && \
  grep "log_debug" /home/user/projects/AAA/swarm/scripts/_common.sh >/dev/null && \
