...
<task type="auto">
  ...
  <action>
    ...
    - SESSION variable: `SESSION="${CLAUDE_SESSION:-swarm-claude-default}"` (env var override)
    - `--session` flag support: optional flag to override session
    - `send` function: accepts `<window> <task_id> "<description>"`, sends `[TASK] {task_id}` then description via tmux send-keys
    - `poll` function: accepts `<window> [timeout] [task_id]`, implements:
      - Default timeout: 30 seconds (documented in usage)
      - While loop that polls until timeout or marker found
      - `tmux capture-pane -t $SESSION:$window -p | tail -200 | grep -E "\[(ACK|DONE|ERROR|WAIT|HELP)\] *$task_id"`
      - Returns first matching line, exits non-zero with "Timeout" message if timeout reached
...
<task type="auto">
  ...
  <action>
    ...
    - Infinite loop that continuously outputs status (may repeat same lines):
      - For each worker window, captures last 200 lines
      - Filters for `[DONE]` or `[ERROR]` markers
      - Outputs timestamped status lines
...
<task type="checkpoint:human-verify">
  ...
  <how-to-verify>
    ...
    **Step 0 - Protocol Setup (Critical):**
    First, send protocol instructions to worker-0 using send-raw (no [TASK] prefix):
    ```bash
    ./scripts/claude_comm.sh send-raw worker-0 "你是 AI Swarm 的 Worker。当收到 [TASK] 开头的消息时：
    1. 立即输出 [ACK] {task_id} 表示收到任务
    2. 完成任务后输出 [DONE] {task_id}
    3. 如果出错输出 [ERROR] {task_id} 错误描述
    4. 不要查任务队列，不要等待外部信号，只需按上述规则回应"
    ```
    Wait a few seconds for Claude to process.

    **Step 1 - Send task:**
    ```bash
    ./scripts/claude_comm.sh send worker-0 task-001 "创建文件 hello.py，内容是 'hello world'。完成后输出 [DONE] task-001"
    ```
    ...
    **Step 2 - Poll ACK:**
    ...
    ```
    Should return line containing `[ACK] task-001`
    ...
    **Step 5 - Verify no Python changes:**
    ```bash
...
## must_haves
  ...
  truths:
    ...
    - "Claude CLI workers need explicit protocol instructions to output [ACK]/[DONE]"
