...
must_haves:
  truths:
    ...
    - "fail subcommand releases lock and logs ERROR status - "Lock with reason"
    conflicts result in exit code 2 with holder information"
...
<task type="auto">
  <name>Create swarm_tasks_bridge.sh script framework</name>
  ...
  <action>
    ...
    3. Usage function with all subcommands documented:
    ...
    4. Main entry point with command parsing
    5. Helper functions: `validate_worker`, `validate_lock_key`
  ...
  <done>
    Script framework exists with help output and subcommand stubs
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    Test claim success:
    ```bash
    # Setup: ensure no existing lock
    rm -f /tmp/ai_swarm/locks/task-test-*.lock 2>/dev/null || true
    ...
    # Verify lock created
    cat /tmp/ai_swarm/locks/task-test-01.lock
    ...
    # Cleanup
    ./scripts/swarm_lock.sh release task-test-conflict worker-1
    ```
  </verify>
  <done>
...
<task type="auto">
  ...
  <verify>
    ...
    # Verify lock removed
    test ! -f /tmp/ai_swarm/locks/task-test-done.lock && echo "Lock released"
...
<task type="auto">
  ...
  <verify>
    ...
    # Verify lock removed
    test ! -f /tmp/ai_swarm/locks/task-test-fail.lock && echo "Lock released"
...
rm -f /tmp/ai_swarm/locks/task-verify-*.lock 2>/dev/null || true
...
# Verify lock exists
echo "Lock exists: $(test -f /tmp/ai_swarm/locks/task-verify-01.lock && echo yes || echo no)"
...
echo "Lock released: $(test ! -f /tmp/ai_swarm/locks/task-verify-01.lock && echo yes || echo no)"
