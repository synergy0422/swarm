...
- README.md: New section after "诊断快照" (line ~173-221)
- SCRIPTS.md: New entry under "任务管理脚本" (line ~350-403)
</context>
...
<task type="auto">
  ...
  <action>
    Add new section "Claude Tasks 协作流程" after the "诊断快照" section (around line 221).
...
<task type="auto">
  ...
  <action>
    Add new entry "swarm_tasks_bridge.sh" under the "任务管理脚本" section (around line 403).
    ...
    Reference existing `swarm_task_wrap.sh` documentation (lines 352-399) for structure.
  </action>
  <verify>
...
<task type="auto">
  ...
  <files>/home/user/projects/AAA/swarm/README.md</files>
  <action>
    Add `swarm_tasks_bridge.sh` to the "Command Mapping" table (around lines 258-266).
