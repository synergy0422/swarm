...
must_haves:
  truths:
    ...
    - "Test is marked @pytest.mark.integration (semi-black-box with mock tmux)"
...
2. **Module marker:**
   ```python
   pytestmark = pytest.mark.integration  # Semi-black-box with mock tmux
   ```
...
def test_press_enter_auto_rescue_mock(mock_tmux_manager):
    ...
    """
    from swarm.auto_rescuer import AutoRescuer, PatternCategory
...
2. **Test runs:** `pytest tests/test_e2e_auto_rescue.py -v -m integration` runs the test
