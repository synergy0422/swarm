...
6. **Option parsing function** - returns count of args consumed:
   ```bash
   parse_options() {
       TTL_SECONDS=""
       SKIP_STATUS=0
       ...
       while [[ $# -gt 0 ]]; do
           case "$1" in
               --ttl)
                   TTL_SECONDS="$2"
                   shift 2
                   OPT_PARSE_COUNT=$((OPT_PARSE_COUNT + 2))
                   ;;
               --no-status)
                   SKIP_STATUS=1
                   shift
                   OPT_PARSE_COUNT=$((OPT_PARSE_COUNT + 1))
                   ;;
               *)
...
7. **Helper commands** with proper parameter handling:
   ```bash
   cmd_acquire_only() {
       local task_id="$1"
...
8. **Core function: cmd_run** - proper option parsing, execute command as array:
   ```bash
   cmd_run() {
       # Parse options first and count consumed args
       parse_options "$@"
       shift $OPT_PARSE_COUNT
