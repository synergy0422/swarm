...
must_haves:
  truths:
    ...
    - "Acquire failure writes WAIT status (no lock release since none held)"
    - "Only releases locks this script acquired (owner validation via swarm_lock.sh)"
...
6. **Helper commands** with proper parameter handling:
   ```bash
   cmd_acquire_only() {
       local task_id="$1"
...
8. **Core function: cmd_run** - options already parsed by main(), execute command as array:
   ```bash
   cmd_run() {
...
9. **Main entry point** with global option parsing:
   ```bash
   main() {
...
- `--ttl SECONDS`: Set lock TTL (applies to run, acquire-only)
- `--no-status`: Skip status logging (applies to all commands)

**Key fixes applied:**
1. **Global option parsing**: Options (--ttl, --no-status) parsed in main() before command dispatch
2. **No eval**: Command executed via `"$@"` instead of `eval "$command"`
3. **Global state**: `LOCK_ACQUIRED`, `LOCK_TASK_ID`, `LOCK_WORKER` declared as globals for trap access
4. **Parameter handling**: skip/wait detect worker-* pattern to resolve args correctly
...
7. **Tests use temp dir**: All tests use $TEST_STATE_DIR for isolation
...
# Test 5: Run success (NO quotes around command!)
...
# Test 6: Run failure (NO quotes around command!)
...
1. **Test that different worker cannot release**:
   ...
   test -f "$TEST_STATE_DIR/locks/test-owner-001.lock" && echo "PASS: Lock not released by wrong worker"
...
2. **Test skip/wait don't release locks** (they don't acquire, so nothing to release):
   ...
   test -f "$TEST_STATE_DIR/locks/test-owner-002.lock" && echo "FAIL: Lock should not exist" || echo "PASS: No lock for skip"
   ...
   test -f "$TEST_STATE_DIR/locks/test-owner-003.lock" && echo "FAIL: Lock should not exist" || echo "PASS: No lock for wait"
   ```

3. **Test that cleanup trap releases only acquired lock**:
   ```bash
   # Start a task that will fail (NO quotes around exit!)
   ./scripts/swarm_task_wrap.sh run test-owner-004 worker-0 exit 1 >/dev/null 2>&1 || true
   # Check lock state
   if [ -f "$TEST_STATE_DIR/locks/test-owner-004.lock" ]; then
...
4. **Verify lock files have worker metadata**:
   ...
   cat "$TEST_STATE_DIR/locks/test-owner-005.lock" | grep -q "worker" && echo "PASS: Lock has worker metadata"
...
# Test 3: Failed task cleanup (NO quotes around exit!)
