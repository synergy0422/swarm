...
# 创建 4 个窗口
tmux new-window -t "$SESSION_NAME" -n "master"
tmux new-window -t "$SESSION_NAME" -n "worker-1"
tmux new-window -t "$SESSION_NAME" -n "worker-2"
tmux new-window -t "$SESSION_NAME" -n "worker-3"

# 窗口布局：水平分割为 4 份
tmux select-layout -t "$SESSION_NAME" tiled

# 启动 Master（在 master 窗口）
tmux send-keys -t "$SESSION_NAME:master" "cd /home/user/projects/AAA/swarm" C-m
tmux send-keys -t "$SESSION_NAME:master" "source .venv/bin/activate" C-m
tmux send-keys -t "$SESSION_NAME:master" "python -m swarm master" C-m

# 启动 Worker 1
tmux send-keys -t "$SESSION_NAME:worker-1" "cd /home/user/projects/AAA/swarm" C-m
tmux send-keys -t "$SESSION_NAME:worker-1" "source .venv/bin/activate" C-m
...
tmux send-keys -t "$SESSION_NAME:worker-2" "source .venv/bin/activate" C-m
tmux send-keys -t "$SESSION_NAME:worker-2" "claude" C-m

# 启动 Worker 3
tmux send-keys -t "$SESSION_NAME:worker-3" "cd /home/user/projects/AAA/swarm" C-m
tmux send-keys -t "$SESSION_NAME:worker-3" "source .venv/bin/activate" C-m
tmux send-keys -t "$SESSION_NAME:worker-3" "claude" C-m
...
```python
# master_poller.py (简化版)
import time
import tmux

def poll_worker_status(session_name, window_name):
    pane = f"{session_name}:{window_name}"
    while True:
        output = tmux.capture_pane(pane)
        if "DONE" in output:
            return "done"
...
| 2. master 窗口运行 Master 进程 | 进入 tmux 查看进程 |
| 3. 3 个 worker 窗口都在运行 claude | 进入 tmux 查看进程 |
...
| 标准 | 说明 |
|------|------|
| 窗口名称正确显示 | tmux 状态栏显示 master/worker-1/worker-2/worker-3 |
| 布局美观 | 4 个窗口均匀分布 |
| 有启动提示 | 启动完成时显示状态概览 |
