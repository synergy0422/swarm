...
# 指定工作目录
./run_claude_swarm.sh --workdir /path/to/project
./run_claude_swarm.sh -d /path/to/project

# 环境变量覆盖
export SWARM_WORKDIR=/path/to/project
./run_claude_swarm.sh
```
...
1. 执行 `./run_claude_swarm.sh --no-attach` 创建 tmux 会话
2. 验证 4 个窗口正确启动 (`tmux list-windows -t swarm-claude-default`)
3. 测试 `--attach` 模式进入交互
4. 测试 `--workdir` 参数覆盖
5. 确认验收标准全部通过
