...
#   claude_comm.sh poll <window> [--task-id ID] [--timeout SECONDS]
...
poll() {
    ...
    while [[ $(date +%s) -lt $deadline ]]; do
        # 捕获最近 200 行，解析标记词
        output=$(tmux capture-pane -t "$SESSION:$window" -p | tail -200)

        if [[ -n "$task_id" ]]; then
            # 按 task_id 过滤
            echo "$output" | grep -E "\[(${task_id})\]"
        else
            # 任意状态标记
            echo "$output" | grep -E '\[(ACK|DONE|ERROR|WAIT|HELP)\]'
        fi
...
./scripts/claude_comm.sh poll worker-0 --task-id task-001 --timeout 10

# 4. 等待 Claude 完成，检查 [DONE]
./scripts/claude_comm.sh poll worker-0 --task-id task-001 --timeout 60
