...
#   claude_comm.sh poll <window> [--timeout N]
...
poll() {
    ...
    # 捕获 pane 输出，解析标记词
    tmux capture-pane -t "$SESSION:$window" -p | grep -E '\[(ACK|DONE|ERROR|WAIT|HELP)\]'
}
...
while true; do
    for w in $WINDOWS; do
        status=$(tmux capture-pane -t "$SESSION:$w" -p | grep -E '\[(DONE|ERROR)\]' | tail -1)
        if [[ -n "$status" ]]; then
...
| # | 标准 | 验证方式 |
|---|------|----------|
| 1 | `claude_comm.sh send worker-0 task-001 "hello"` 发送成功 | tmux show-buffer 验证 |
| 2 | Worker 返回 `[ACK] task-001` | `claude_comm.sh poll worker-0` |
| 3 | Worker 返回 `[DONE] task-001` | `claude_comm.sh poll worker-0` |
...
# 3. 检查 worker-0 是否回复 [ACK]
./scripts/claude_comm.sh poll worker-0
